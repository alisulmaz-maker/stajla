...
connectToDb().then(() => {
    ...
    app.post('/api/register', async (req, res) => {
        try {
            ...
            if (filter.isProfane(name)) { // DOĞRU FONKSİYON KULLANILDI
    ...
    app.post('/api/ogrenci-ilan', upload.single('cv'), async (req, res) => {
        ...
        try {
            ...
            if (filter.isProfane(yeniIlan.name) || filter.isProfane(yeniIlan.desc) || filter.isProfane(yeniIlan.dept)) { // DOĞRU FONKSİYON KULLANILDI
    ...
    app.post('/api/isveren-ilan', async (req, res) => {
        ...
        try {
            const yeniIlan = req.body;
            if (filter.isProfane(yeniIlan.company) || filter.isProfane(yeniIlan.sector) || filter.isProfane(yeniIlan.req)) { // DOĞRU FONKSİYON KULLANILDI
                return res.status(400).json({ success: false, message: 'İlan içeriğinde uygun olmayan kelimeler tespit edildi.' });
            }
            yeniIlan.createdBy = new ObjectId(req.session.user.id);
